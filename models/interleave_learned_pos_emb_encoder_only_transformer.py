...
class InterleaveLearnedPosEmbEncoderOnlyTransformer(nn.Module):
    ''' The New Transformer module '''
    def __init__(self, config, dataset):
        ''' Initialize the Transformer '''
        super(InterleaveLearnedPosEmbEncoderOnlyTransformer, self).__init__()
        ...
        for i in range((self.num_layers+1)//2):
    ...
    def encode(self, inputs):
        ''' Encode the inputs '''
        word_embedding = self.encoder_embed(inputs, self.embedding)
        encoded = {
        ...
        for i, encoder in enumerate(self.encoders):
            ...
            if i < self.num_layers -1:
                if i % 2 == 1:
                    encoded['state'] += self.encoder_positional_embeddings[(i+1)//2](inputs)
                else:
                    encoded['state'] += self.position_embedding(inputs)
    ...
    def decode(self, encoded, targets, decoders=None, embedding=None, cache=None, mask=None, input_lens=None):
        ...
        word_embedding = self.decoder_embed(targets, embedding)
    ...
    def encoder_embed(self, inputs, token_embedding):
        ''' Embed the given inputs '''
        return self.dropout(token_embedding(inputs) + self.encoder_positional_embeddings[0](inputs))
    
    def decoder_embed(self, inputs, token_embedding):
        ''' Embed the given inputs '''
        return self.dropout(token_embedding(inputs) + self.position_embedding_decoder(inputs, True))
