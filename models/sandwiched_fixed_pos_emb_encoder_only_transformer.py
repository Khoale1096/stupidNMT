...
class SandwichedFixedPosEmbEncoderOnlyTransformer(nn.Module):
    ''' The New Transformer module '''
    def __init__(self, config, dataset):
        ''' Initialize the Transformer '''
        super(SandwichedFixedPosEmbEncoderOnlyTransformer, self).__init__()
        ...
        for i, encoder in enumerate(self.encoders):
            ...
            if i < self.num_layers -1:
                if i == self.num_layers - 2:
                    ...
                    encoded['state'] += self.encoder_positional_embeddings[i](inputs)
