...
class SingleLearnedPosEmbTransformer(nn.Module):
    ''' The New Transformer module '''
    def __init__(self, config, dataset):
        ''' Initialize the Transformer '''
        super(SingleLearnedPosEmbTransformer, self).__init__()
    ...
    def encode(self, inputs):
        ''' Encode the inputs '''
        word_embedding = self.encoder_pos_embed(inputs, self.embedding)
        encoded = {
    ...
    def decode(self, encoded, targets, decoders=None, embedding=None, cache=None, mask=None, input_lens=None):
        ...
        word_embedding = self.decoder_pos_embed(targets, embedding)
    ...
    def encoder_pos_embed(self, inputs, token_embedding):
        ''' Embed the given inputs '''
    ...
    def decoder_pos_embed(self, inputs, token_embedding):
        ''' Embed the given inputs '''
        position_ids = [list(range(inputs.shape[1])) for i in range(inputs.shape[0])]
